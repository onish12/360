...
if($r.IndexOf('|Set-Content',[StringComparison]::OrdinalIgnoreCase)-ge0 -or $r.IndexOf('| Set-Content',[StringComparison]::OrdinalIgnoreCase)-ge0){throw 'H15OR_PIPELINE_SETCONTENT_FORBIDDEN'}
foreach($x in @('function WriteUtf8','function WriteAsciiLines','RemoveSubject $ProbeSubject')){if($r.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "H15OR_RUN_MISSING_SAFE_IO:$x"}}
foreach($x in @("'H15OR_READONLY_RECOVERY_PACKAGE'",'$ResultBytes=120','H15OR_DIRTY_OR_NONBASELINE_STATE_DETECTED_H15O_RETAINED',"'/delete-driver'",'if($safe -and -not$err)')){if($r.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "H15OR_RUN_MISSING:$x"}}
