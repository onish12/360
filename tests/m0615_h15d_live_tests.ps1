...
foreach($x in @("[Convert]::ToUInt32('8338E458',16)","[Convert]::ToUInt32('00000010',16)","[Convert]::ToUInt32('807B0DFF',16)","[Convert]::ToUInt32('00000014',16)","[Convert]::ToUInt32('807B0DFD',16)",'GENERIC_READ|GENERIC_WRITE','DeviceIoControl H15D transaction','MANIFEST_TRANSACTION_CONTRACT_MISMATCH','CODE_SIGNING_EKU_MISSING','PHASER_DIAGNOSTIC_FILTER_ALREADY_ATTACHED','$ioAttempted=$true','((-not $ioAttempted) -or $ioComplete)','TRUST_RETAINED_FOR_SAFETY=TRUE','WRITE_RESTORE_COMPLETED','BASELINE_RESTORED','TRUST_RESTORED','PciConfigWrite=''ONLY_0x44_BIT2_AND_0x48_BIT1_WITH_EXACT_RESTORE''','Mmio=''NO''','DspBoot=''NO''')){if($run.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "H15D_RUNNER_MISSING: $x"}}
