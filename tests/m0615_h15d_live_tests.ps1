...
foreach($x in @('ReadConfigImage(bus,liveConfig)','RtlCompareMemory(liveConfig,evidence.config,kPciConfigSnapshotBytes)','gate.Removed()?STATUS_DELETE_PENDING')){if($policy.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "H15D_POLICY_MISSING: $x"}}
