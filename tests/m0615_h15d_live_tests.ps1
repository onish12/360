...
foreach($x in @('0x8338e458u','0x00000010u','0x807b0dffu','0x00000014u','0x807b0dfdu','kH15dRequiredSuccessFlags=0x3ffu','sizeof(H15dLiveResultV1)==52u')){if($hdr.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "H15D_HDR_MISSING: $x"}}
...
foreach($x in @("[Convert]::ToUInt32('8338E458',16)","[Convert]::ToUInt32('00000010',16)","[Convert]::ToUInt32('807B0DFF',16)","[Convert]::ToUInt32('00000014',16)","[Convert]::ToUInt32('807B0DFD',16)",'GENERIC_READ|GENERIC_WRITE','DeviceIoControl H15D transaction','WRITE_RESTORE_COMPLETED','BASELINE_RESTORED','TRUST_RESTORED','PciConfigWrite=''ONLY_0x44_BIT2_AND_0x48_BIT1_WITH_EXACT_RESTORE''','Mmio=''NO''','DspBoot=''NO''')){if($run.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "H15D_RUNNER_MISSING: $x"}}
...
foreach($x in @('-KeyExportPolicy NonExportable','PHASER360_H15D_LIVE_R1_BOUNDED_PCI_TRANSACTION_PACKAGE',"PciConfigWrite='ONLY_0x44_BIT2_AND_0x48_BIT1_WITH_EXACT_RESTORE'",'PrivateKeyExported=$false')){if($wf.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "H15D_WORKFLOW_MISSING: $x"}}
