...
foreach($x in @(
 'm1-fast-safe-20260923-r1','EXACT_WINDOWS_BUILD_19044_REQUIRED','PCI\VEN_8086&DEV_3198&SUBSYS_00000000&REV_06',
 ...
 '[Phaser360.M1FastNative]::Query($TelemetryGuid,$TelemetryIoctl,32)','Start-Sleep -Milliseconds 500',
...
foreach($x in @('-NonInteractive','Run-M1FastSafe.ps1','m1-fast-safe-20260923-r1')){if($c.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "M1_FAST_CMD_MISSING:$x"}}
...
Write-Host 'M1_FAST_SAFE_STATIC_TESTS=PASS; exact_target=YES; baseline_backup_before_bind=YES; recovery_pointer_before_bind=YES; telemetry=DOUBLE_READ; rollback=AUTOMATIC; playback=NO; reboot=NO'
