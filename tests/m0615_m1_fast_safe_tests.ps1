...
foreach($x in @('phaser360_dma_owner_tests','phaser360_glk_boot_tests','phaser360_stream_tests','phaser360_rom_tests','M1_R7_HOST_TESTS_FAILED')){
 if($w.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R7_WORKFLOW_GUARD_MISSING:$x"}
}
foreach($bad in @('M1_BIND_REQUIRES_REBOOT','INTEL_FALLBACK_REQUIRES_REBOOT')){
 if($r.IndexOf($bad,[StringComparison]::Ordinal)-ge0){throw "M1_R7_REBOOT_EARLY_ABORT_FORBIDDEN:$bad"}
}
foreach($x in @('$bindRebootSignalled','bind_update_result.json','BindRebootSignalled=$bindRebootSignalled','$fallbackRebootSignalled','intel_fallback_update_result.json','IntelFallbackRebootSignalled=$fallbackRebootSignalled')){
 if($r.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R7_REBOOT_EVIDENCE_MISSING:$x"}
}
foreach($src in @($hc,$hs)){
 if($src.IndexOf('next>=0x1000',[StringComparison]::Ordinal)-ge0 -or $src.IndexOf('0x1000-', [StringComparison]::Ordinal)-ge0){throw 'M1_R7_LEGACY_CAP_WINDOW_PRESENT'}
}
foreach($x in @('0x0c00','0x0800','0x0500','0x1f00','0x0700')){
 if($st.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "M1_R7_PHYSICAL_CAP_CHAIN_TEST_MISSING:$x"}
}
...
foreach($x in @(
 ...
 if($p.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "M1_R7_PCI_LIVE_CONTRACT_GUARD_MISSING:$x"}
}
foreach($x in @(
 ...
 if($h.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "M1_R7_PCI_TRACE_GUARD_MISSING:$x"}
}
if($p.IndexOf('RtlCompareMemory(liveConfig,evidence.config,kPciConfigSnapshotBytes)',[StringComparison]::OrdinalIgnoreCase)-ge0){
 throw 'M1_R7_FULL_256_BYTE_LIVE_FREEZE_FORBIDDEN'
}
foreach($x in @('pciConfig[0x52]^=1u','pciConfig[0x51]=0','Owned dword changing')){
 if($gt.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "M1_R7_PCI_REGRESSION_MISSING:$x"}
}
foreach($x in @('RomPhase::InitPreAdspcs','RomPhase::InitPowerUpCores','RomPhase::InitRunCore0','RomPhase::InitWaitRomReady')){
 if($rom.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R7_ROM_PHASE_GUARD_MISSING:$x"}
}
...
foreach($x in @('P20_DMA_PREPARE_ENTER','P25_DMA_PREPARE_OK')){
 if($d.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R7_DMA_TRACE_MISSING:$x"}
}
foreach($x in @('D20_LIFECYCLE_D0_ENTER','D40_FIRMWARE_COMMAND_READY','D50_FRAMEWORK_ENABLE_GRANT_OK','POST20_OK')){
 if($l.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R7_LIFECYCLE_TRACE_MISSING:$x"}
}
foreach($x in @('H10_HDA_PREPARE_ENTER','H90_HDA_PREPARE_OK')){
 if($h.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R7_HDA_TRACE_MISSING:$x"}
}
foreach($x in @('G10_GLK_PREPARE_ENTER','T20_HDA_START_OK','T30_ROM_ENTERED_OK','T50_IPC_READY_OK','T60_COMMAND_READY_OK')){
 if($g.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R7_GLK_TRACE_MISSING:$x"}
}
