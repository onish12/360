...
foreach($x in @(
 'm1-fast-safe-20260924-r5-cap-chain-fix','EXACT_WINDOWS_BUILD_19044_REQUIRED','PCI\VEN_8086&DEV_3198&SUBSYS_00000000&REV_06',
 ...
 'ebaed0db-f9db-42ea-a162-5f4111384051','StartStageTrace','StopStageTrace','M1_R5_STAGE_TRACE.etl','M1_TARGET_FAILED_START_CODE10','DEVPKEY_Device_ProblemStatus',
...
foreach($x in @('-NonInteractive','Run-M1FastSafe.ps1','m1-fast-safe-20260924-r5-cap-chain-fix')){if($c.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "M1_FAST_CMD_MISSING:$x"}}
foreach($x in @('verify-sof-reference.py','generate-embedded-firmware.py','phaser360_m1_boot.vcxproj','Inf2Cat.exe','New-SelfSignedCertificate','KeyExportPolicy NonExportable','phaser360_m1_fast_safe.cer','package_manifest.json','upload-artifact','M1_FAST_SAFE_R5_CAP_CHAIN_FIX_DSP_BOOT','0.6.15.134')){if($w.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "M1_FAST_WORKFLOW_MISSING:$x"}}
...
foreach($x in @('0x0c00','0x0800','0x0500','0x1f00','0x0700')){
 if($st.IndexOf($x,[StringComparison]::OrdinalIgnoreCase)-lt0){throw "M1_R5_PHYSICAL_CAP_CHAIN_TEST_MISSING:$x"}
}
foreach($x in @('EtwRegister','EtwWriteString','EtwUnregister','PHASER360_R5 ')){
 if($s.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R5_STAGE_TRACE_PROVIDER_MISSING:$x"}
}
foreach($x in @('StageTraceRegister','Phaser360EvtDriverUnload','A00_DRIVER_ENTRY','A20_DEVICE_OWNER_OK')){
 if($e.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R5_DRIVER_ENTRY_TRACE_MISSING:$x"}
}
foreach($x in @('P20_DMA_PREPARE_ENTER','P25_DMA_PREPARE_OK')){
 if($d.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R5_DMA_TRACE_MISSING:$x"}
}
foreach($x in @('D20_LIFECYCLE_D0_ENTER','D40_FIRMWARE_COMMAND_READY','D50_FRAMEWORK_ENABLE_GRANT_OK','POST20_OK')){
 if($l.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R5_LIFECYCLE_TRACE_MISSING:$x"}
}
foreach($x in @('H10_HDA_PREPARE_ENTER','H90_HDA_PREPARE_OK')){
 if($h.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R5_HDA_TRACE_MISSING:$x"}
}
foreach($x in @('G10_GLK_PREPARE_ENTER','T20_HDA_START_OK','T30_ROM_ENTERED_OK','T50_IPC_READY_OK','T60_COMMAND_READY_OK')){
 if($g.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R5_GLK_TRACE_MISSING:$x"}
}
foreach($x in @('I10_INTERRUPT_ENABLE_ENTER','I20_MASK_GLOBAL_OK','I30_MASK_LOCAL_OK','I40_INTERRUPT_ENABLE_OK')){
 if($i.IndexOf($x,[StringComparison]::Ordinal)-lt0){throw "M1_R5_IRQ_TRACE_MISSING:$x"}
}

Write-Host 'M1_FAST_SAFE_STATIC_TESTS=PASS; r5_cap_chain_fix=YES; stage_trace=ETW_PRE_BIND; exact_target=YES; baseline_backup_before_bind=YES; recovery_pointer_before_bind=YES; telemetry=DOUBLE_READ; dma_lifetime=PREPARE_TO_RELEASE; rollback=AUTOMATIC; playback=NO; reboot=NO'
