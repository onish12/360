...
$Service='Phaser360H15i'
$CertSubject='CN=PHASER360 H15I Ephemeral Test Signing'
$InterfaceGuid=[Guid]'8c1b3150-6d12-4f88-9d36-15f300319801'
$Ioctl=[Convert]::ToUInt32('833DE46C',16)
$RequiredFlags=[Convert]::ToUInt32('0003FFFF',16)
$ResultBytes=160
...
function Published {
  ...
  foreach($f in @(Get-ChildItem (Join-Path $env:SystemRoot 'INF') -Filter 'oem*.inf' -File)){
    ...
    if($t.IndexOf('Phaser360H15i',[StringComparison]::OrdinalIgnoreCase) -ge 0 -and
...
function Package([string]$root,[switch]$Trusted){
  ...
  if([string]$m.Purpose -cne 'H15I_HDA_CRST_TRANSACTION_PACKAGE' -or
    ...
    throw 'MANIFEST_H15I_CONTRACT_MISMATCH'
  }
  foreach($x in @(
...
function Native {
  if('Phaser360.H15iNative' -as [type]){return}
  $src=@'
using System;
...
namespace Phaser360 {
  public static class H15iNative {
    ...
    static string InterfacePath(Guid g) {
      ...
      try {
        ...
        if(second||Marshal.GetLastWin32Error()!=ERROR_NO_MORE_ITEMS)
          throw new InvalidOperationException("EXPECTED_EXACTLY_ONE_H15I_INTERFACE");
        uint n=0;
    ...
    public static byte[] Transaction(Guid g,uint ioctl,byte[] input,int bytes) {
      ...
      using(var h=CreateFile(path,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ|FILE_SHARE_WRITE,IntPtr.Zero,OPEN_EXISTING,0,IntPtr.Zero)) {
        if(h.IsInvalid) throw new Win32Exception(Marshal.GetLastWin32Error(),"CreateFile H15I");
        var o=new byte[bytes]; uint got;
        Q(DeviceIoControl(h,ioctl,input,(uint)input.Length,o,(uint)o.Length,out got,IntPtr.Zero),"DeviceIoControl H15I");
        if(got!=bytes) throw new InvalidOperationException("H15I_RESULT_SIZE="+got);
        return o;
      }
    }
  }
...
if(@(Published).Count -ne 0){throw 'H15I_PACKAGE_ALREADY_PRESENT'}
...
$tb=Trust $pkg.Thumb
if($tb.Root -or $tb.TrustedPublisher){throw 'H15I_CERT_ALREADY_TRUSTED'}

if([string]::IsNullOrWhiteSpace($OutputRoot)){$OutputRoot=$PSScriptRoot}
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$suffix=[Guid]::NewGuid().ToString('N').Substring(0,8)
$dir=Join-Path $OutputRoot ('H15I_TRANSACTION_'+$stamp+'_'+$suffix)
...
Copy-Item (Join-Path $PSScriptRoot 'H15I_WINRE_ROLLBACK.txt') $dir
...
try {
  ...
  $x.Output|Set-Content (Join-Path $dir 'pnputil_publish_h15i.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15I_DRIVERSTORE_PUBLISH_FAILED'}
  $p=@(Published)
  if($p.Count -ne 1){throw "EXPECTED_ONE_H15I_PUBLISHED_INF: count=$($p.Count)"}
  $publishedInf=$p[0]
  $published=$true
  $publishedInf|Set-Content (Join-Path $dir 'H15iPublishedInf.txt') -Encoding ASCII

  Native
  $handoffAttempted=$true
  $reboot=[Phaser360.H15iNative]::ForceUpdate($ExactHwid,$pkg.Inf)
  if($reboot){throw 'H15I_HANDOFF_REQUIRES_SYSTEM_REBOOT'}

  $with=WaitService $before.InstanceId $Service 20
  $with|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'target_with_h15i.json') -Encoding UTF8
  if($with.DriverInfPath -cne $publishedInf){
    throw "H15I_PUBLISHED_INF_NOT_ACTIVE: active=$($with.DriverInfPath) published=$publishedInf"
  }
  ...
  $writeAttempted=$true
  $r=[Phaser360.H15iNative]::Transaction($InterfaceGuid,$Ioctl,$req,$ResultBytes)
  ...
  function RegSet([byte[]]$b,[int]$o){
    [ordered]@{
      HdaGcap=('0x{0:X4}' -f (U16 $b $o))
      HdaVmin=('0x{0:X2}' -f [uint32]$b[$o+2])
      HdaVmaj=('0x{0:X2}' -f [uint32]$b[$o+3])
      HdaGctl=('0x{0:X8}' -f (U32 $b ($o+4)))
      HdaIntelEm2=('0x{0:X8}' -f (U32 $b ($o+8)))
      DspAdspcs=('0x{0:X8}' -f (U32 $b ($o+12)))
      DspAdspis=('0x{0:X8}' -f (U32 $b ($o+16)))
      DspHipci=('0x{0:X8}' -f (U32 $b ($o+20)))
      DspHipcie=('0x{0:X8}' -f (U32 $b ($o+24)))
      DspRomStatus=('0x{0:X8}' -f (U32 $b ($o+28)))
    }
  }
  $beforeRegs=RegSet $r 64
  $readyRegs=RegSet $r 96
  $restoredRegs=RegSet $r 128
  ...
  }|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'h15i_transaction.json') -Encoding UTF8

  $beforeGctl=U32 $r 68
  $readyGctl=U32 $r 100
  $restoredGctl=U32 $r 132
  if($v -ne 1 -or $sz -ne 160 -or $nt -lt 0 -or
     ...
     $beforeGctl -ne 0 -or ($readyGctl -band 1) -ne 1 -or
     $restoredGctl -ne $beforeGctl){
    throw 'H15I_LIVE_TRANSACTION_VALIDATION_FAILED'
  }
  $writeRestoreComplete=$true
  $snapshotComplete=$true

  $x=PnP @('/delete-driver',$publishedInf,'/uninstall','/force')
  $x.Output|Set-Content (Join-Path $dir 'pnputil_remove_h15i.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15I_UNINSTALL_FAILED'}
  ...
  if(@(Published).Count -ne 0){throw 'H15I_PACKAGE_REMAINS'}
  ...
  if(-not $normal){
    ...
    if($published -and ((-not $writeAttempted) -or $writeRestoreComplete)){
      ...
      $log+='USE_H15I_WINRE_ROLLBACK_IF_WINDOWS_BECOMES_UNBOOTABLE=TRUE'
    }
...
[ordered]@{
  ...
  FunctionDriver='Phaser360H15i'
  Mapping='HDA_PAGE_READWRITE_DSP_PAGE_READONLY'
  MmioRead='REVIEWED_REGISTERS_ONLY';MmioWrite='ONLY_HDA_GCTL_CRST_BIT0_0_TO_1_TO_0'
...
Hashes $dir
$zip=Join-Path $OutputRoot ('RESULT_H15I_TRANSACTION_'+$stamp+'_'+$suffix+'.zip')
