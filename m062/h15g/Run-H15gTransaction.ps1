...
$Service='Phaser360H15g'
$CertSubject='CN=PHASER360 H15G Ephemeral Test Signing'
$InterfaceGuid=[Guid]'8c1b3150-6d10-4f88-9d36-15f100319801'
$Ioctl=[Convert]::ToUInt32('833B6464',16)
$RequiredFlags=[Convert]::ToUInt32('00001FFF',16)
$SnapshotBytes=120
...
function Published {
  ...
  foreach($f in @(Get-ChildItem (Join-Path $env:SystemRoot 'INF') -Filter 'oem*.inf' -File)){
    ...
    if($t.IndexOf('Phaser360H15g',[StringComparison]::OrdinalIgnoreCase) -ge 0 -and
...
function CodeIntegrity {
  if(-not ('Phaser360.H15gCi' -as [type])){
    Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;namespace Phaser360{public static class H15gCi{[StructLayout(LayoutKind.Sequential)]struct CI{public UInt32 Length;public UInt32 Options;}[DllImport("ntdll.dll")]static extern Int32 NtQuerySystemInformation(Int32 c,ref CI i,UInt32 l,IntPtr r);public static UInt32 Get(){CI i=new CI();i.Length=(UInt32)Marshal.SizeOf(typeof(CI));Int32 s=NtQuerySystemInformation(103,ref i,i.Length,IntPtr.Zero);if(s<0)throw new InvalidOperationException("CI=0x"+unchecked((UInt32)s).ToString("X8"));return i.Options;}}}' -Language CSharp
  }
  [uint32][Phaser360.H15gCi]::Get()
}
function Package([string]$root,[switch]$Trusted){
  $r=(Resolve-Path $root).Path
  $inf=Join-Path $r 'phaser360_h15g_function_mmio.inf'
  $sys=Join-Path $r 'phaser360_h15g_function_mmio.sys'
  $cat=Join-Path $r 'phaser360_h15g_function_mmio.cat'
  $cer=Join-Path $r 'phaser360_h15g_function_mmio.cer'
  ...
  if([string]$m.Purpose -cne 'H15G_FUNCTION_DRIVER_READONLY_MMIO_PACKAGE' -or
    ...
    throw 'MANIFEST_H15G_CONTRACT_MISMATCH'
  }
  foreach($x in @(
...
function Native {
  if('Phaser360.H15gNative' -as [type]){return}
  $src=@'
using System;
...
namespace Phaser360 {
  public static class H15gNative {
    ...
    static string InterfacePath(Guid g) {
      ...
      try {
        ...
        if(second||Marshal.GetLastWin32Error()!=ERROR_NO_MORE_ITEMS)
          throw new InvalidOperationException("EXPECTED_EXACTLY_ONE_H15G_INTERFACE");
        uint n=0;
    ...
    public static byte[] Snapshot(Guid g,uint ioctl,int bytes) {
      ...
      using(var h=CreateFile(path,GENERIC_READ,FILE_SHARE_READ|FILE_SHARE_WRITE,IntPtr.Zero,OPEN_EXISTING,0,IntPtr.Zero)) {
        if(h.IsInvalid) throw new Win32Exception(Marshal.GetLastWin32Error(),"CreateFile H15G");
        var o=new byte[bytes]; uint got;
        Q(DeviceIoControl(h,ioctl,IntPtr.Zero,0,o,(uint)o.Length,out got,IntPtr.Zero),"DeviceIoControl H15G");
        if(got!=bytes) throw new InvalidOperationException("H15G_SNAPSHOT_SIZE="+got);
        return o;
      }
    }
  }
...
if(@(Published).Count -ne 0){throw 'H15G_PACKAGE_ALREADY_PRESENT'}
...
$tb=Trust $pkg.Thumb
if($tb.Root -or $tb.TrustedPublisher){throw 'H15G_CERT_ALREADY_TRUSTED'}

if([string]::IsNullOrWhiteSpace($OutputRoot)){$OutputRoot=$PSScriptRoot}
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$suffix=[Guid]::NewGuid().ToString('N').Substring(0,8)
$dir=Join-Path $OutputRoot ('H15G_TRANSACTION_'+$stamp+'_'+$suffix)
...
Copy-Item (Join-Path $PSScriptRoot 'H15G_WINRE_ROLLBACK.txt') $dir
...
try {
  ...
  $x.Output|Set-Content (Join-Path $dir 'pnputil_publish_h15g.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15G_DRIVERSTORE_PUBLISH_FAILED'}
  $p=@(Published)
  if($p.Count -ne 1){throw "EXPECTED_ONE_H15G_PUBLISHED_INF: count=$($p.Count)"}
  $publishedInf=$p[0]
  $published=$true
  $publishedInf|Set-Content (Join-Path $dir 'H15gPublishedInf.txt') -Encoding ASCII

  Native
  $handoffAttempted=$true
  $reboot=[Phaser360.H15gNative]::ForceUpdate($ExactHwid,$pkg.Inf)
  if($reboot){throw 'H15G_HANDOFF_REQUIRES_SYSTEM_REBOOT'}

  $with=WaitService $before.InstanceId $Service 20
  $with|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'target_with_h15g.json') -Encoding UTF8
  if($with.DriverInfPath -cne $publishedInf){
    throw "H15G_PUBLISHED_INF_NOT_ACTIVE: active=$($with.DriverInfPath) published=$publishedInf"
  }
  $handoffComplete=$true

  $r=[Phaser360.H15gNative]::Snapshot($InterfaceGuid,$Ioctl,$SnapshotBytes)
  $v=U32 $r 0
  $sz=U32 $r 4
  $fl=U32 $r 8
  $nt=I32 $r 12
  ...
  [ordered]@{
    ...
    HdaGcap=('0x{0:X4}' -f $gcap)
    HdaVmin=('0x{0:X2}' -f $vmin)
    HdaVmaj=('0x{0:X2}' -f $vmaj)
    HdaGctl=('0x{0:X8}' -f $gctl)
    HdaIntelEm2=('0x{0:X8}' -f $em2)
    DspAdspcs=('0x{0:X8}' -f $adspcs)
    DspAdspis=('0x{0:X8}' -f $adspis)
    DspHipci=('0x{0:X8}' -f $hipci)
    DspHipcie=('0x{0:X8}' -f $hipcie)
    DspRomStatus=('0x{0:X8}' -f $rom)
  }|ConvertTo-Json -Depth 6|Set-Content (Join-Path $dir 'h15g_snapshot.json') -Encoding UTF8

  if($v -ne 1 -or $sz -ne 120 -or $nt -lt 0 -or
     ...
     $gcap -eq 0xffff -or $gctl -eq 0xffffffff -or
     $em2 -eq 0xffffffff -or $adspcs -eq 0xffffffff -or
     $adspis -eq 0xffffffff){
    throw 'H15G_LIVE_SNAPSHOT_VALIDATION_FAILED'
  }
  $snapshotComplete=$true

  $x=PnP @('/delete-driver',$publishedInf,'/uninstall','/force')
  $x.Output|Set-Content (Join-Path $dir 'pnputil_remove_h15g.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15G_UNINSTALL_FAILED'}
  ...
  if(@(Published).Count -ne 0){throw 'H15G_PACKAGE_REMAINS'}
...
}else{'H15G_FUNCTION_MMIO_CAPTURE_FAILED'}
...
[ordered]@{
  ...
  FunctionDriver='Phaser360H15g'
  Mapping='PAGE_READONLY_NOCACHE_THROUGH_D0'
  MmioRead='REVIEWED_REGISTERS_ONLY';MmioWrite='NO';PciWrite='NO';Dma='NO'
...
Hashes $dir
$zip=Join-Path $OutputRoot ('RESULT_H15G_TRANSACTION_'+$stamp+'_'+$suffix+'.zip')
