...
$ready=$ci.TestSignAllowed -and $re.ExitCode -eq 0 -and $re.Status -ceq 'ENABLED' -and
       -not $trust.Root -and -not $trust.TrustedPublisher
$status=if($ready){'H15C_LIVE_R2_INSTALL_READY'}else{'H15C_LIVE_R2_INSTALL_BLOCKED'}

if([string]::IsNullOrWhiteSpace($OutputRoot)){$OutputRoot=$PSScriptRoot}
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$suffix=[Guid]::NewGuid().ToString('N').Substring(0,8)
$dir=Join-Path $OutputRoot ('H15C_LIVE_R2_PREFLIGHT_'+$stamp+'_'+$suffix)
...
$trust|ConvertTo-Json -Depth 4|Set-Content (Join-Path $dir 'trust_before.json') -Encoding UTF8
...
$result=[ordered]@{
    ...
    PackageSignerThumbprint=$package.CertificateThumbprint
    PackageCertSubject=$package.CertificateSubject
    PackageCertNotAfter=$package.CertificateNotAfter
    PackageSysAuthenticode=$package.SysAuthenticode
    PackageCatAuthenticode=$package.CatAuthenticode
    CertificateAlreadyInRoot=$trust.Root
    CertificateAlreadyInTrustedPublisher=$trust.TrustedPublisher
    PackageInfSha256=$package.InfSha256;PackageSysSha256=$package.SysSha256
    PackageCatSha256=$package.CatSha256;PackageCertSha256=$package.CertificateSha256
    DriverInstall='NO';DeviceRestart='NO';TrustChange='NO_PREFLIGHT';BcdWrite='NO'
...
$zip=Join-Path $OutputRoot ('RESULT_H15C_LIVE_R2_PREFLIGHT_'+$stamp+'_'+$suffix+'.zip')
...
Write-Host "CERT_ROOT_PRESENT=$($trust.Root.ToString().ToUpperInvariant())"
Write-Host "CERT_TRUSTEDPUBLISHER_PRESENT=$($trust.TrustedPublisher.ToString().ToUpperInvariant())"
