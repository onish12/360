...
$Service='Phaser360H15k'
$CertSubject='CN=PHASER360 H15K Ephemeral Test Signing'
$InterfaceGuid=[Guid]'8c1b3150-6d12-4f88-9d36-15f400319802'
$Ioctl=[Convert]::ToUInt32('833DE474',16)
$RequiredFlags=[Convert]::ToUInt32('03FFFFFF',16)
$ResultBytes=256
...
function Published {
  ...
  foreach($f in @(Get-ChildItem (Join-Path $env:SystemRoot 'INF') -Filter 'oem*.inf' -File)){
    ...
    if($t.IndexOf('Phaser360H15k',[StringComparison]::OrdinalIgnoreCase) -ge 0 -and
...
function CodeIntegrity {
  if(-not ('Phaser360.H15kCi' -as [type])){
    Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;namespace Phaser360{public static class H15kCi{[StructLayout(LayoutKind.Sequential)]struct CI{public UInt32 Length;public UInt32 Options;}[DllImport("ntdll.dll")]static extern Int32 NtQuerySystemInformation(Int32 c,ref CI i,UInt32 l,IntPtr r);public static UInt32 Get(){CI i=new CI();i.Length=(UInt32)Marshal.SizeOf(typeof(CI));Int32 s=NtQuerySystemInformation(103,ref i,i.Length,IntPtr.Zero);if(s<0)throw new InvalidOperationException("CI=0x"+unchecked((UInt32)s).ToString("X8"));return i.Options;}}}' -Language CSharp
  }
  [uint32][Phaser360.H15kCi]::Get()
}
function Package([string]$root,[switch]$Trusted){
  $r=(Resolve-Path $root).Path
  $inf=Join-Path $r 'phaser360_h15k_adspcs_reset_gate.inf'
  $sys=Join-Path $r 'phaser360_h15k_adspcs_reset_gate.sys'
  $cat=Join-Path $r 'phaser360_h15k_adspcs_reset_gate.cat'
  $cer=Join-Path $r 'phaser360_h15k_adspcs_reset_gate.cer'
  ...
  if([string]$m.Purpose -cne 'H15K_ADSPCS_STALL_RESET_TRANSACTION_PACKAGE' -or
    ...
    throw 'MANIFEST_H15K_CONTRACT_MISMATCH'
  }
  foreach($x in @(
...
function Native {
  if('Phaser360.H15kNative' -as [type]){return}
  $src=@'
using System;
...
namespace Phaser360 {
  public static class H15kNative {
    ...
    static string InterfacePath(Guid g) {
      ...
      try {
        ...
        if(second||Marshal.GetLastWin32Error()!=ERROR_NO_MORE_ITEMS)
          throw new InvalidOperationException("EXPECTED_EXACTLY_ONE_H15K_INTERFACE");
        uint n=0;
    ...
    public static byte[] Transaction(Guid g,uint ioctl,byte[] input,int bytes) {
      ...
      using(var h=CreateFile(path,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ|FILE_SHARE_WRITE,IntPtr.Zero,OPEN_EXISTING,0,IntPtr.Zero)) {
        if(h.IsInvalid) throw new Win32Exception(Marshal.GetLastWin32Error(),"CreateFile H15K");
        var o=new byte[bytes]; uint got;
        Q(DeviceIoControl(h,ioctl,input,(uint)input.Length,o,(uint)o.Length,out got,IntPtr.Zero),"DeviceIoControl H15K");
        if(got!=bytes) throw new InvalidOperationException("H15K_RESULT_SIZE="+got);
        return o;
      }
    }
  }
...
if(@(Published).Count -ne 0){throw 'H15K_PACKAGE_ALREADY_PRESENT'}
...
$tb=Trust $pkg.Thumb
if($tb.Root -or $tb.TrustedPublisher){throw 'H15K_CERT_ALREADY_TRUSTED'}

if([string]::IsNullOrWhiteSpace($OutputRoot)){$OutputRoot=$PSScriptRoot}
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$suffix=[Guid]::NewGuid().ToString('N').Substring(0,8)
$dir=Join-Path $OutputRoot ('H15K_TRANSACTION_'+$stamp+'_'+$suffix)
...
Copy-Item (Join-Path $PSScriptRoot 'H15K_WINRE_ROLLBACK.txt') $dir
...
try {
  ...
  $x.Output|Set-Content (Join-Path $dir 'pnputil_publish_h15k.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15K_DRIVERSTORE_PUBLISH_FAILED'}
  $p=@(Published)
  if($p.Count -ne 1){throw "EXPECTED_ONE_H15K_PUBLISHED_INF: count=$($p.Count)"}
  $publishedInf=$p[0]
  $published=$true
  $publishedInf|Set-Content (Join-Path $dir 'H15kPublishedInf.txt') -Encoding ASCII

  Native
  $handoffAttempted=$true
  $reboot=[Phaser360.H15kNative]::ForceUpdate($ExactHwid,$pkg.Inf)
  if($reboot){throw 'H15K_HANDOFF_REQUIRES_SYSTEM_REBOOT'}

  $with=WaitService $before.InstanceId $Service 20
  $with|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'target_with_h15k.json') -Encoding UTF8
  if($with.DriverInfPath -cne $publishedInf){
    throw "H15K_PUBLISHED_INF_NOT_ACTIVE: active=$($with.DriverInfPath) published=$publishedInf"
  }
  ...
  $writeAttempted=$true
  $r=[Phaser360.H15kNative]::Transaction($InterfaceGuid,$Ioctl,$req,$ResultBytes)
  ...
  [ordered]@{
    ...
    Before=$beforeRegs;Stalled=$stalledRegs;Reset=$resetRegs;Restored=$restoredRegs
  }|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'h15k_transaction.json') -Encoding UTF8
  ...
  $stalledAdspcs=U32 $r 132
  $resetAdspcs=U32 $r 180
  $restoredAdspcs=U32 $r 228

  # Establish rollback proof before the broader validator. This prevents a
  # parser/metadata error from blocking cleanup after exact hardware restore.
  $writeAttempted=(($fl -band [Convert]::ToUInt32('00000010',16)) -ne 0)
  $writeRestoreComplete=(($fl -band [Convert]::ToUInt32('0000C000',16)) -eq [Convert]::ToUInt32('0000C000',16) -and
  ...
  if($v -ne 1 -or $sz -ne $ResultBytes -or $nt -lt 0 -or
     ...
     $stalledAdspcs -ne [Convert]::ToUInt32('001D033C',16) -or
     $resetAdspcs -ne [Convert]::ToUInt32('001D033F',16) -or
     $restoredAdspcs -ne $beforeAdspcs){
    throw 'H15K_LIVE_TRANSACTION_VALIDATION_FAILED'
  }
  $snapshotComplete=$true

  $x=PnP @('/delete-driver',$publishedInf,'/uninstall','/force')
  $x.Output|Set-Content (Join-Path $dir 'pnputil_remove_h15k.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15K_UNINSTALL_FAILED'}
  ...
  if(@(Published).Count -ne 0){throw 'H15K_PACKAGE_REMAINS'}
...
}else{'H15K_ADSPCS_TRANSACTION_FAILED'}
...
[ordered]@{
  ...
  FunctionDriver='Phaser360H15k'
  Mapping='HDA_PAGE_READONLY_DSP_PAGE_READWRITE'
  MmioRead='HDA_IDLE_AND_DSP_STATUS';MmioWrite='ONLY_DSP_ADSPCS_CSTALL_CRST_CORES01_WITH_EXACT_ROLLBACK'
  ...
  HdaMmioWrite='NO';SpaCpaWrite='NO'
...
Hashes $dir
$zip=Join-Path $OutputRoot ('RESULT_H15K_TRANSACTION_'+$stamp+'_'+$suffix+'.zip')
