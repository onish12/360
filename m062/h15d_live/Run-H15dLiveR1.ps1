...
try{
 ...
 $x=CertUtil @('-delstore','TrustedPublisher',$pkg.Thumb);if($x.ExitCode -ne 0){throw 'CERT_PUBLISHER_REMOVE_FAILED'};$pubAdded=$false
 $x=CertUtil @('-delstore','Root',$pkg.Thumb);if($x.ExitCode -ne 0){throw 'CERT_ROOT_REMOVE_FAILED'};$rootAdded=$false
