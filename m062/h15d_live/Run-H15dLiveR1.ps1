...
function Target {
 ...
 $upper=@();try{$u=@(Get-PnpDeviceProperty -InstanceId $id -KeyName 'DEVPKEY_Device_CompoundUpperFilters' -ErrorAction Stop);if($u.Count -eq 1){$upper=@($u[0].Data)}}catch{}
...
function AssertTarget($s){
 ...
 if($s.Status -cne 'OK' -or $s.ProblemCode -ne 0 -or $s.Service -cne 'IntcAudioBus'){throw 'TARGET_NOT_HEALTHY_ON_INTC_AUDIO_BUS'}
 if(@($s.HardwareIds|Where-Object {$_ -ceq $ExactHwid}).Count -ne 1){throw 'EXACT_HARDWARE_ID_NOT_PRESENT'}
}
...
function Published {
 $r=@();foreach($f in @(Get-ChildItem (Join-Path $env:SystemRoot 'INF') -Filter 'oem*.inf' -File)){try{$t=Get-Content $f.FullName -Raw}catch{continue};if($t.IndexOf($ExtensionId,[StringComparison]::OrdinalIgnoreCase) -ge 0 -and $t.IndexOf($Service,[StringComparison]::OrdinalIgnoreCase) -ge 0){$r+=$f.Name}};@($r)
}
...
function Package([string]$root,[switch]$Trusted){
 ...
 $eku=@();foreach($e in $c.Extensions){if($e.Oid.Value -eq '2.5.29.37'){$te=[System.Security.Cryptography.X509Certificates.X509EnhancedKeyUsageExtension]$e;foreach($o in $te.EnhancedKeyUsages){$eku+=[string]$o.Value}}};if(@($eku|Where-Object {$_ -ceq '1.3.6.1.5.5.7.3.3'}).Count -ne 1){throw 'CODE_SIGNING_EKU_MISSING'}
 ...
 $it=Get-Content $inf -Raw;if($it.IndexOf($ExtensionId,[StringComparison]::OrdinalIgnoreCase) -lt 0 -or $it.IndexOf($ExactHwid,[StringComparison]::OrdinalIgnoreCase) -lt 0){throw 'PACKAGE_INF_IDENTITY_MISMATCH'}
...
try{
 ...
 $x=PnP @('/restart-device',$before.InstanceId);if($x.ExitCode -ne 0){throw 'BASELINE_DEVICE_RESTART_FAILED'};$after=WaitHealthy $before.InstanceId
 if(@(Published).Count -ne 0){throw 'H15D_FILTER_PACKAGE_REMAINS'}
 if(@($after.CompoundUpperFilters|Where-Object {$_ -ceq $Service}).Count -ne 0){throw 'H15D_FILTER_REMAINS_IN_COMPOUND_UPPER_FILTERS'}
 ...
 $x=CertUtil @('-delstore','TrustedPublisher',$pkg.Thumb);if($x.ExitCode -ne 0){throw 'CERT_PUBLISHER_REMOVE_FAILED'}
 $ta=Trust $pkg.Thumb;if($ta.TrustedPublisher){throw 'CERT_PUBLISHER_REMAINS'};$pubAdded=$false
 $x=CertUtil @('-delstore','Root',$pkg.Thumb);if($x.ExitCode -ne 0){throw 'CERT_ROOT_REMOVE_FAILED'}
 $ta=Trust $pkg.Thumb;if($ta.Root){throw 'CERT_ROOT_REMAINS'};$rootAdded=$false
 if($ta.Root -or $ta.TrustedPublisher){throw 'CERT_TRUST_REMAINS'};$after|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'target_after.json') -Encoding UTF8;$normal=$true
 ...
 $safe=$false;try{$s=Target;$safe=((-not $ioAttempted) -or $ioComplete) -and @(Published).Count -eq 0 -and $s.Status -ceq 'OK' -and $s.ProblemCode -eq 0 -and $s.InstanceId -ceq $before.InstanceId -and $s.Service -ceq $before.Service -and $s.DriverInfPath -ceq $before.DriverInfPath -and $s.DriverVersion -ceq $before.DriverVersion -and $s.DriverProvider -ceq $before.DriverProvider -and @($s.HardwareIds|Where-Object {$_ -ceq $ExactHwid}).Count -eq 1 -and @($s.CompoundUpperFilters|Where-Object {$_ -ceq $Service}).Count -eq 0}catch{}
 if($safe){if($pubAdded){$q=CertUtil @('-delstore','TrustedPublisher',$pkg.Thumb);$qp=Trust $pkg.Thumb;if($q.ExitCode -eq 0 -and -not $qp.TrustedPublisher){$pubAdded=$false}};if($rootAdded){$q=CertUtil @('-delstore','Root',$pkg.Thumb);$qr=Trust $pkg.Thumb;if($q.ExitCode -eq 0 -and -not $qr.Root){$rootAdded=$false}}}else{$log+='TRUST_RETAINED_FOR_SAFETY=TRUE'}
 $log|Set-Content (Join-Path $dir 'emergency_rollback.txt') -Encoding UTF8
 }
}
$final=$null;try{$final=Target}catch{};$rem=@(Published);$ft=Trust $pkg.Thumb;$baseline=$false;if($final){$baseline=$final.Status -ceq 'OK' -and $final.ProblemCode -eq 0 -and $final.InstanceId -ceq $before.InstanceId -and $final.Service -ceq $before.Service -and $final.DriverInfPath -ceq $before.DriverInfPath -and $final.DriverVersion -ceq $before.DriverVersion -and $final.DriverProvider -ceq $before.DriverProvider -and @($final.HardwareIds|Where-Object {$_ -ceq $ExactHwid}).Count -eq 1 -and @($final.CompoundUpperFilters|Where-Object {$_ -ceq $Service}).Count -eq 0 -and $rem.Count -eq 0 -and -not $ft.Root -and -not $ft.TrustedPublisher}
