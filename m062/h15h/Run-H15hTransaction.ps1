...
$Service='Phaser360H15h'
$CertSubject='CN=PHASER360 H15H Ephemeral Test Signing'
$InterfaceGuid=[Guid]'8c1b3150-6d11-4f88-9d36-15f200319801'
$Ioctl=[Convert]::ToUInt32('833CE468',16)
$RequiredFlags=[Convert]::ToUInt32('0001FFFF',16)
$ResultBytes=176
...
function Published {
  ...
  foreach($f in @(Get-ChildItem (Join-Path $env:SystemRoot 'INF') -Filter 'oem*.inf' -File)){
    ...
    if($t.IndexOf('Phaser360H15h',[StringComparison]::OrdinalIgnoreCase) -ge 0 -and
...
function CodeIntegrity {
  if(-not ('Phaser360.H15hCi' -as [type])){
    Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;namespace Phaser360{public static class H15hCi{[StructLayout(LayoutKind.Sequential)]struct CI{public UInt32 Length;public UInt32 Options;}[DllImport("ntdll.dll")]static extern Int32 NtQuerySystemInformation(Int32 c,ref CI i,UInt32 l,IntPtr r);public static UInt32 Get(){CI i=new CI();i.Length=(UInt32)Marshal.SizeOf(typeof(CI));Int32 s=NtQuerySystemInformation(103,ref i,i.Length,IntPtr.Zero);if(s<0)throw new InvalidOperationException("CI=0x"+unchecked((UInt32)s).ToString("X8"));return i.Options;}}}' -Language CSharp
  }
  [uint32][Phaser360.H15hCi]::Get()
}
function Package([string]$root,[switch]$Trusted){
  $r=(Resolve-Path $root).Path
  $inf=Join-Path $r 'phaser360_h15h_function_pci_policy.inf'
  $sys=Join-Path $r 'phaser360_h15h_function_pci_policy.sys'
  $cat=Join-Path $r 'phaser360_h15h_function_pci_policy.cat'
  $cer=Join-Path $r 'phaser360_h15h_function_pci_policy.cer'
  ...
  if([string]$m.Purpose -cne 'H15H_FUNCTION_DRIVER_PCI_POLICY_PACKAGE' -or
...
function Native {
  if('Phaser360.H15hNative' -as [type]){return}
  $src=@'
using System;
...
namespace Phaser360 {
  public static class H15hNative {
    ...
    static string InterfacePath(Guid g) {
      ...
      try {
        ...
        if(second||Marshal.GetLastWin32Error()!=ERROR_NO_MORE_ITEMS)
          throw new InvalidOperationException("EXPECTED_EXACTLY_ONE_H15H_INTERFACE");
        uint n=0;
    ...
    public static byte[] Transaction(Guid g,uint ioctl,byte[] input,int bytes) {
      ...
      using(var h=CreateFile(path,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ|FILE_SHARE_WRITE,IntPtr.Zero,OPEN_EXISTING,0,IntPtr.Zero)) {
        if(h.IsInvalid) throw new Win32Exception(Marshal.GetLastWin32Error(),"CreateFile H15H");
        var o=new byte[bytes]; uint got;
        Q(DeviceIoControl(h,ioctl,input,(uint)input.Length,o,(uint)o.Length,out got,IntPtr.Zero),"DeviceIoControl H15H");
        if(got!=bytes) throw new InvalidOperationException("H15H_RESULT_SIZE="+got);
        return o;
      }
    }
  }
...
if(@(Published).Count -ne 0){throw 'H15H_PACKAGE_ALREADY_PRESENT'}
...
$tb=Trust $pkg.Thumb
if($tb.Root -or $tb.TrustedPublisher){throw 'H15H_CERT_ALREADY_TRUSTED'}

if([string]::IsNullOrWhiteSpace($OutputRoot)){$OutputRoot=$PSScriptRoot}
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$suffix=[Guid]::NewGuid().ToString('N').Substring(0,8)
$dir=Join-Path $OutputRoot ('H15H_TRANSACTION_'+$stamp+'_'+$suffix)
...
Copy-Item (Join-Path $PSScriptRoot 'H15H_WINRE_ROLLBACK.txt') $dir
...
try {
  ...
  $x.Output|Set-Content (Join-Path $dir 'pnputil_publish_h15h.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15H_DRIVERSTORE_PUBLISH_FAILED'}
  $p=@(Published)
  if($p.Count -ne 1){throw "EXPECTED_ONE_H15H_PUBLISHED_INF: count=$($p.Count)"}
  $publishedInf=$p[0]
  $published=$true
  $publishedInf|Set-Content (Join-Path $dir 'H15hPublishedInf.txt') -Encoding ASCII

  Native
  $handoffAttempted=$true
  $reboot=[Phaser360.H15hNative]::ForceUpdate($ExactHwid,$pkg.Inf)
  if($reboot){throw 'H15H_HANDOFF_REQUIRES_SYSTEM_REBOOT'}

  $with=WaitService $before.InstanceId $Service 20
  $with|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'target_with_h15h.json') -Encoding UTF8
  if($with.DriverInfPath -cne $publishedInf){
    throw "H15H_PUBLISHED_INF_NOT_ACTIVE: active=$($with.DriverInfPath) published=$publishedInf"
  }
  ...
  [ordered]@{
    ...
    PgBefore=('0x{0:X8}' -f $pg0);CgBefore=('0x{0:X8}' -f $cg0)
    PgApplied=('0x{0:X8}' -f $pg1);CgApplied=('0x{0:X8}' -f $cg1)
    PgRestored=('0x{0:X8}' -f $pg2);CgRestored=('0x{0:X8}' -f $cg2)
    HdaPhysical=('0x{0:X16}' -f $hda);DspPhysical=('0x{0:X16}' -f $dsp)
    HdaLength=('0x{0:X8}' -f $hdaLen);DspLength=('0x{0:X8}' -f $dspLen)
    Before=(RegSet $r 80)
    Applied=(RegSet $r 112)
    Restored=(RegSet $r 144)
  }|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'h15h_transaction.json') -Encoding UTF8

  if($v -ne 1 -or $sz -ne 176 -or $nt -lt 0 -or
     ($fl -band $RequiredFlags) -ne $RequiredFlags -or
     $ven -ne 0x8086 -or $dev -ne 0x3198 -or
     $pg0 -ne $ExpectedPg -or $cg0 -ne $ExpectedCg -or
     $pg1 -ne $AppliedPg -or $cg1 -ne $AppliedCg -or
     $pg2 -ne $ExpectedPg -or $cg2 -ne $ExpectedCg -or
  ...
  $x.Output|Set-Content (Join-Path $dir 'pnputil_remove_h15h.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15H_UNINSTALL_FAILED'}
  ...
  if(@(Published).Count -ne 0){throw 'H15H_PACKAGE_REMAINS'}
...
}else{'H15H_FUNCTION_PCI_POLICY_FAILED'}
...
[ordered]@{
  ...
  FunctionDriver='Phaser360H15h'
  Mapping='PAGE_READONLY_NOCACHE_THROUGH_D0'
  MmioRead='REVIEWED_REGISTERS_ONLY';MmioWrite='NO'
  PciConfigWrite='ONLY_0x44_BIT2_AND_0x48_BIT1_WITH_EXACT_RESTORE';Dma='NO'
...
Hashes $dir
$zip=Join-Path $OutputRoot ('RESULT_H15H_TRANSACTION_'+$stamp+'_'+$suffix+'.zip')
