...
const GUID phaser360::windows::kH15jInterfaceGuid={
    0x8c1b3150,0x6d12,0x4f88,{0x9d,0x36,0x15,0xf3,0x00,0x31,0x98,0x02}
};
...
bool ReadObservation(H15jDeviceContext* c,H15jObservation* r) noexcept {
    auto* g=Gate(c);
    if(KeGetCurrentIrql()!=PASSIVE_LEVEL || !c || !r || !g || !g->Allowed() || g->Removed() ||
       !c->hda || !c->dsp || c->hdaLength!=kH15jHdaBytes || c->dspLength!=kH15jDspBytes) return false;
    ...
    r->totalStreams=static_cast<UCHAR>(total); r->streamRunMask=0;
    for(ULONG i=0;i<total;++i){ const UCHAR ctl=READ_REGISTER_UCHAR(c->hda+kStreamBase+i*kStreamStride); if(ctl&kRunBit) r->streamRunMask|=(1u<<i); }
    ...
    return r->hdaGcap!=0xffffu && r->hdaGctl!=0xffffffffu && r->hdaIntelEm2!=0xffffffffu &&
           r->dspAdspcs!=0xffffffffu && r->dspAdspic!=0xffffffffu && r->dspAdspis!=0xffffffffu && r->dspHipcctl!=0xffffffffu;
}
bool ExactKnownBaseline(const H15jObservation& r) noexcept {
    return r.hdaGcap==0x6701u && r.hdaVmin==0u && r.hdaVmaj==1u && r.hdaGctl==0u && r.totalStreams==13u &&
           r.hdaIntelEm2==0x04007000u && r.dspAdspcs==0x001d003cu && r.dspAdspis==0u &&
           r.dspHipci==0u && r.dspHipcie==0x00420000u && r.dspRomStatus==0x01006701u;
}
bool HdaQuiescent(const H15jObservation& r) noexcept {
    return (r.hdaGctl&kGctlCrst)!=0 && (r.hdaCorbctl&kRunBit)==0 && (r.hdaRirbctl&kRunBit)==0 &&
           r.streamRunMask==0 && r.totalStreams==13u;
}
...
void phaser360::windows::H15jEvtIoDeviceControl(
    ...
    result.flags=H15jNoPciWrite|H15jNoDspMmioWrite|H15jNoDma|
        H15jNoIrqOwnership|H15jNoFirmware|H15jNoDspBoot|H15jOneShot|H15jSplitMappings|H15jNoHdaNonGctlWrite;
    ...
    if(NT_SUCCESS(status)&&pci.Valid()){
        ...
        if(p.pgctl==kH15jExpectedPgctl && p.cgctl==kH15jExpectedCgctl &&
           ReadObservation(c,&result.before) && ExactKnownBaseline(result.before)){
            result.flags|=H15jExpectedBaselineMatch|H15jBeforeCaptured;
            ...
            if(DelayUs(500) && WriteGctlCrst(c,true)){
                ...
                if(PollGctlCrst(c,true)){
                    ...
                    if(DelayUs(1000) && ReadObservation(c,&result.ready)){
                        result.flags|=H15jReadyCaptured;
                        if(HdaQuiescent(result.ready)){ readyQuiescent=true; result.flags|=H15jHdaQuiescentObserved; }
                    }
                }
            }
