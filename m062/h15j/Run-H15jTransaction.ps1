...
$Service='Phaser360H15j'
$CertSubject='CN=PHASER360 H15J Ephemeral Test Signing'
$InterfaceGuid=[Guid]'8c1b3150-6d12-4f88-9d36-15f300319802'
$Ioctl=[Convert]::ToUInt32('833DE470',16)
$RequiredFlags=[Convert]::ToUInt32('000FFFFF',16)
$ResultBytes=208
...
function Published {
  ...
  foreach($f in @(Get-ChildItem (Join-Path $env:SystemRoot 'INF') -Filter 'oem*.inf' -File)){
    ...
    if($t.IndexOf('Phaser360H15j',[StringComparison]::OrdinalIgnoreCase) -ge 0 -and
...
function CodeIntegrity {
  if(-not ('Phaser360.H15jCi' -as [type])){
    Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;namespace Phaser360{public static class H15jCi{[StructLayout(LayoutKind.Sequential)]struct CI{public UInt32 Length;public UInt32 Options;}[DllImport("ntdll.dll")]static extern Int32 NtQuerySystemInformation(Int32 c,ref CI i,UInt32 l,IntPtr r);public static UInt32 Get(){CI i=new CI();i.Length=(UInt32)Marshal.SizeOf(typeof(CI));Int32 s=NtQuerySystemInformation(103,ref i,i.Length,IntPtr.Zero);if(s<0)throw new InvalidOperationException("CI=0x"+unchecked((UInt32)s).ToString("X8"));return i.Options;}}}' -Language CSharp
  }
  [uint32][Phaser360.H15jCi]::Get()
}
function Package([string]$root,[switch]$Trusted){
  $r=(Resolve-Path $root).Path
  $inf=Join-Path $r 'phaser360_h15j_hda_crst_gate.inf'
  $sys=Join-Path $r 'phaser360_h15j_hda_crst_gate.sys'
  $cat=Join-Path $r 'phaser360_h15j_hda_crst_gate.cat'
  $cer=Join-Path $r 'phaser360_h15j_hda_crst_gate.cer'
  ...
  if([string]$m.Purpose -cne 'H15J_HDA_QUIESCENCE_TRANSACTION_PACKAGE' -or
    ...
    throw 'MANIFEST_H15J_CONTRACT_MISMATCH'
  }
  foreach($x in @(
...
function Native {
  if('Phaser360.H15jNative' -as [type]){return}
  $src=@'
using System;
...
namespace Phaser360 {
  public static class H15jNative {
    ...
    static string InterfacePath(Guid g) {
      ...
      try {
        ...
        if(second||Marshal.GetLastWin32Error()!=ERROR_NO_MORE_ITEMS)
          throw new InvalidOperationException("EXPECTED_EXACTLY_ONE_H15J_INTERFACE");
        uint n=0;
    ...
    public static byte[] Transaction(Guid g,uint ioctl,byte[] input,int bytes) {
      ...
      using(var h=CreateFile(path,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ|FILE_SHARE_WRITE,IntPtr.Zero,OPEN_EXISTING,0,IntPtr.Zero)) {
        if(h.IsInvalid) throw new Win32Exception(Marshal.GetLastWin32Error(),"CreateFile H15J");
        var o=new byte[bytes]; uint got;
        Q(DeviceIoControl(h,ioctl,input,(uint)input.Length,o,(uint)o.Length,out got,IntPtr.Zero),"DeviceIoControl H15J");
        if(got!=bytes) throw new InvalidOperationException("H15J_RESULT_SIZE="+got);
        return o;
      }
    }
  }
...
if(@(Published).Count -ne 0){throw 'H15J_PACKAGE_ALREADY_PRESENT'}
...
$tb=Trust $pkg.Thumb
if($tb.Root -or $tb.TrustedPublisher){throw 'H15J_CERT_ALREADY_TRUSTED'}

if([string]::IsNullOrWhiteSpace($OutputRoot)){$OutputRoot=$PSScriptRoot}
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$suffix=[Guid]::NewGuid().ToString('N').Substring(0,8)
$dir=Join-Path $OutputRoot ('H15J_TRANSACTION_'+$stamp+'_'+$suffix)
...
Copy-Item (Join-Path $PSScriptRoot 'H15J_WINRE_ROLLBACK.txt') $dir
...
try {
  ...
  $x.Output|Set-Content (Join-Path $dir 'pnputil_publish_h15j.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15J_DRIVERSTORE_PUBLISH_FAILED'}
  $p=@(Published)
  if($p.Count -ne 1){throw "EXPECTED_ONE_H15J_PUBLISHED_INF: count=$($p.Count)"}
  $publishedInf=$p[0]
  $published=$true
  $publishedInf|Set-Content (Join-Path $dir 'H15jPublishedInf.txt') -Encoding ASCII

  Native
  $handoffAttempted=$true
  $reboot=[Phaser360.H15jNative]::ForceUpdate($ExactHwid,$pkg.Inf)
  if($reboot){throw 'H15J_HANDOFF_REQUIRES_SYSTEM_REBOOT'}

  $with=WaitService $before.InstanceId $Service 20
  $with|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'target_with_h15j.json') -Encoding UTF8
  if($with.DriverInfPath -cne $publishedInf){
    throw "H15J_PUBLISHED_INF_NOT_ACTIVE: active=$($with.DriverInfPath) published=$publishedInf"
  }
  ...
  $writeAttempted=$true
  $r=[Phaser360.H15jNative]::Transaction($InterfaceGuid,$Ioctl,$req,$ResultBytes)
  ...
  }|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'h15j_transaction.json') -Encoding UTF8

  $beforeGctl=U32 $r 68
  $readyGctl=U32 $r 116
  $readyCorb=[uint32]$r[120]
  $readyRirb=[uint32]$r[121]
  ...
  if($v -ne 1 -or $sz -ne 160 -or $nt -lt 0 -or
     ...
     ($readyCorb -band 2) -ne 0 -or ($readyRirb -band 2) -ne 0 -or
     $readyStreams -ne 13 -or $readyRunMask -ne 0 -or
     $restoredGctl -ne $beforeGctl){
    throw 'H15J_LIVE_TRANSACTION_VALIDATION_FAILED'
  }
  $writeRestoreComplete=$true
  $snapshotComplete=$true

  $x=PnP @('/delete-driver',$publishedInf,'/uninstall','/force')
  $x.Output|Set-Content (Join-Path $dir 'pnputil_remove_h15j.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15J_UNINSTALL_FAILED'}
  ...
  if(@(Published).Count -ne 0){throw 'H15J_PACKAGE_REMAINS'}
  ...
  if(-not $normal){
    ...
    if($published -and ((-not $writeAttempted) -or $writeRestoreComplete)){
      ...
      $log+='USE_H15J_WINRE_ROLLBACK_IF_WINDOWS_BECOMES_UNBOOTABLE=TRUE'
    }
...
[ordered]@{
  ...
  FunctionDriver='Phaser360H15j'
  Mapping='HDA_PAGE_READWRITE_DSP_PAGE_READONLY'
  MmioRead='HDA_QUIESCENCE_AND_DSP_STATUS';MmioWrite='ONLY_HDA_GCTL_CRST_BIT0_0_TO_1_TO_0'
  HdaQuiescence='CORB_RIRB_ALL_STREAM_RUN_ZERO_BEFORE_CRST_CLEAR'
...
Hashes $dir
$zip=Join-Path $OutputRoot ('RESULT_H15J_TRANSACTION_'+$stamp+'_'+$suffix+'.zip')
