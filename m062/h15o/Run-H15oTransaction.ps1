...
$Service='Phaser360H15o'
$CertSubject='CN=PHASER360 H15O Ephemeral Test Signing'
$InterfaceGuid=[Guid]'8c1b3150-6d12-4f88-9d36-15f900319805'
$Ioctl=[Convert]::ToUInt32('8342E488',16)
$RequiredFlags=[Convert]::ToUInt32('FFDFFFFF',16)
$ResultBytes=464
...
function Published {
  ...
  foreach($f in @(Get-ChildItem (Join-Path $env:SystemRoot 'INF') -Filter 'oem*.inf' -File)){
    ...
    if($t.IndexOf('Phaser360H15o',[StringComparison]::OrdinalIgnoreCase) -ge 0 -and
...
function CodeIntegrity {
  if(-not ('Phaser360.H15oCi' -as [type])){
    Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;namespace Phaser360{public static class H15oCi{[StructLayout(LayoutKind.Sequential)]struct CI{public UInt32 Length;public UInt32 Options;}[DllImport("ntdll.dll")]static extern Int32 NtQuerySystemInformation(Int32 c,ref CI i,UInt32 l,IntPtr r);public static UInt32 Get(){CI i=new CI();i.Length=(UInt32)Marshal.SizeOf(typeof(CI));Int32 s=NtQuerySystemInformation(103,ref i,i.Length,IntPtr.Zero);if(s<0)throw new InvalidOperationException("CI=0x"+unchecked((UInt32)s).ToString("X8"));return i.Options;}}}' -Language CSharp
  }
  [uint32][Phaser360.H15oCi]::Get()
}
function Package([string]$root,[switch]$Trusted){
  $r=(Resolve-Path $root).Path
  $inf=Join-Path $r 'phaser360_h15o_staged_pre_fw_policy_gate.inf'
  $sys=Join-Path $r 'phaser360_h15o_staged_pre_fw_policy_gate.sys'
  $cat=Join-Path $r 'phaser360_h15o_staged_pre_fw_policy_gate.cat'
  $cer=Join-Path $r 'phaser360_h15o_staged_pre_fw_policy_gate.cer'
  ...
  if([string]$m.Purpose -cne 'H15O_STAGED_PRE_FW_POLICY_TRANSACTION_PACKAGE' -or
     ...
     [string]$m.MmioWrite -cne 'ONLY_HDA_GCTL_CRST_PPCTL_GPROCEN_EM2_L1SEN_WITH_EXACT_ROLLBACK' -or
     [string]$m.PpctlWrite -cne 'ONLY_GPROCEN_BIT30_SET_CLEAR' -or
     [string]$m.Em2Write -cne 'ONLY_L1SEN_BIT13_CLEAR_SET' -or
     [string]$m.DspMmioWrite -cne 'NO' -or
     [string]$m.PciConfigWrite -cne 'ONLY_CGCTL_0x48_BIT1_AND_PGCTL_0x44_BIT2_WITH_EXACT_ROLLBACK' -or
     [string]$m.ExpectedPgctl -cne '0x00000010' -or
     [string]$m.ExpectedCgctl -cne '0x807B0DFF'){
    throw 'MANIFEST_H15O_CONTRACT_MISMATCH'
  }
  foreach($x in @(
...
function Native {
  if('Phaser360.H15oNative' -as [type]){return}
  $src=@'
using System;
...
namespace Phaser360 {
  public static class H15oNative {
    ...
    static string InterfacePath(Guid g) {
      ...
      try {
        ...
        if(second||Marshal.GetLastWin32Error()!=ERROR_NO_MORE_ITEMS)
          throw new InvalidOperationException("EXPECTED_EXACTLY_ONE_H15O_INTERFACE");
        uint n=0;
    ...
    public static byte[] Transaction(Guid g,uint ioctl,byte[] input,int bytes) {
      ...
      using(var h=CreateFile(path,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ|FILE_SHARE_WRITE,IntPtr.Zero,OPEN_EXISTING,0,IntPtr.Zero)) {
        if(h.IsInvalid) throw new Win32Exception(Marshal.GetLastWin32Error(),"CreateFile H15O");
        var o=new byte[bytes]; uint got;
        Q(DeviceIoControl(h,ioctl,input,(uint)input.Length,o,(uint)o.Length,out got,IntPtr.Zero),"DeviceIoControl H15O");
        if(got!=bytes) throw new InvalidOperationException("H15O_RESULT_SIZE="+got);
        return o;
      }
    }
  }
...
if(@(Published).Count -ne 0){throw 'H15O_PACKAGE_ALREADY_PRESENT'}
...
$tb=Trust $pkg.Thumb
if($tb.Root -or $tb.TrustedPublisher){throw 'H15O_CERT_ALREADY_TRUSTED'}

if([string]::IsNullOrWhiteSpace($OutputRoot)){$OutputRoot=$PSScriptRoot}
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$suffix=[Guid]::NewGuid().ToString('N').Substring(0,8)
$dir=Join-Path $OutputRoot ('H15O_TRANSACTION_'+$stamp+'_'+$suffix)
...
Copy-Item (Join-Path $PSScriptRoot 'H15O_WINRE_ROLLBACK.txt') $dir
...
try {
  ...
  $x.Output|Set-Content (Join-Path $dir 'pnputil_publish_h15o.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15O_DRIVERSTORE_PUBLISH_FAILED'}
  $p=@(Published)
  if($p.Count -ne 1){throw "EXPECTED_ONE_H15O_PUBLISHED_INF: count=$($p.Count)"}
  $publishedInf=$p[0]
  $published=$true
  $publishedInf|Set-Content (Join-Path $dir 'H15oPublishedInf.txt') -Encoding ASCII

  Native
  $handoffAttempted=$true
  $reboot=[Phaser360.H15oNative]::ForceUpdate($ExactHwid,$pkg.Inf)
  if($reboot){throw 'H15O_HANDOFF_REQUIRES_SYSTEM_REBOOT'}

  $with=WaitService $before.InstanceId $Service 20
  $with|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'target_with_h15o.json') -Encoding UTF8
  if($with.DriverInfPath -cne $publishedInf){
    throw "H15O_PUBLISHED_INF_NOT_ACTIVE: active=$($with.DriverInfPath) published=$publishedInf"
  }
  ...
  $writeAttempted=$true
  $r=[Phaser360.H15oNative]::Transaction($InterfaceGuid,$Ioctl,$req,$ResultBytes)

  $v=U32 $r 0;$sz=U32 $r 4;$nt=I32 $r 8;$fl=U32 $r 12;$gen=U32 $r 16
  $ven=U16 $r 20;$dev=U16 $r 22;$pg=U32 $r 28;$cg=U32 $r 32;$hda=U64 $r 40;$dsp=U64 $r 48;$hdaLen=U32 $r 56;$dspLen=U32 $r 60
  function Obs([byte[]]$b,[int]$o){[ordered]@{
    HdaGcap=('0x{0:X4}' -f (U16 $b $o));HdaVmin=('0x{0:X2}' -f [uint32]$b[$o+2]);HdaVmaj=('0x{0:X2}' -f [uint32]$b[$o+3]);HdaGctl=('0x{0:X8}' -f (U32 $b ($o+4)))
    HdaCorbctl=('0x{0:X2}' -f [uint32]$b[$o+8]);HdaRirbctl=('0x{0:X2}' -f [uint32]$b[$o+9]);TotalStreams=[uint32]$b[$o+10];StreamRunMask=('0x{0:X8}' -f (U32 $b ($o+12)))
    HdaIntelEm2=('0x{0:X8}' -f (U32 $b ($o+16)));DspAdspcs=('0x{0:X8}' -f (U32 $b ($o+20)));DspAdspic=('0x{0:X8}' -f (U32 $b ($o+24)));DspAdspis=('0x{0:X8}' -f (U32 $b ($o+28)))
    DspHipci=('0x{0:X8}' -f (U32 $b ($o+32)));DspHipcie=('0x{0:X8}' -f (U32 $b ($o+36)));DspHipcctl=('0x{0:X8}' -f (U32 $b ($o+40)));DspRomStatus=('0x{0:X8}' -f (U32 $b ($o+44)))
  }}
  $beforeRegs=Obs $r 64;$readyRegs=Obs $r 112;$gprocenRegs=Obs $r 160;$cgRegs=Obs $r 208;$em2Regs=Obs $r 256;$pgRegs=Obs $r 304;$finalRegs=Obs $r 352
  $llch=U32 $r 400;$ppOffset=U32 $r 404;$ppHeader=U32 $r 408;$ppctlBefore=U32 $r 412;$ppstsBefore=U32 $r 416;$ppctlEnabled=U32 $r 420;$ppstsEnabled=U32 $r 424
  $ppctlRestored=U32 $r 428;$ppstsRestored=U32 $r 432;$pgAfterCg=U32 $r 436;$cgAfterCg=U32 $r 440;$pgAfterPg=U32 $r 444;$cgAfterPg=U32 $r 448;$pgRestored=U32 $r 452;$cgRestored=U32 $r 456;$cpaStageMask=U32 $r 460
  [ordered]@{Version=$v;Size=$sz;NtStatus=('0x{0:X8}' -f ([uint32]$nt));Flags=('0x{0:X8}' -f $fl);Generation=$gen;Vendor=('0x{0:X4}' -f $ven);Device=('0x{0:X4}' -f $dev)
    Pgctl=('0x{0:X8}' -f $pg);Cgctl=('0x{0:X8}' -f $cg);HdaPhysical=('0x{0:X16}' -f $hda);DspPhysical=('0x{0:X16}' -f $dsp);HdaLength=('0x{0:X8}' -f $hdaLen);DspLength=('0x{0:X8}' -f $dspLen)
    Before=$beforeRegs;Ready=$readyRegs;GprocenOnly=$gprocenRegs;CgOff=$cgRegs;Em2Off=$em2Regs;PgOn=$pgRegs;Final=$finalRegs
    Llch=('0x{0:X8}' -f $llch);PpOffset=('0x{0:X4}' -f $ppOffset);PpHeader=('0x{0:X8}' -f $ppHeader);PpctlBefore=('0x{0:X8}' -f $ppctlBefore);PpstsBefore=('0x{0:X8}' -f $ppstsBefore)
    PpctlEnabled=('0x{0:X8}' -f $ppctlEnabled);PpstsEnabled=('0x{0:X8}' -f $ppstsEnabled);PpctlRestored=('0x{0:X8}' -f $ppctlRestored);PpstsRestored=('0x{0:X8}' -f $ppstsRestored)
    PgAfterCg=('0x{0:X8}' -f $pgAfterCg);CgAfterCg=('0x{0:X8}' -f $cgAfterCg);PgAfterPg=('0x{0:X8}' -f $pgAfterPg);CgAfterPg=('0x{0:X8}' -f $cgAfterPg);PgRestored=('0x{0:X8}' -f $pgRestored);CgRestored=('0x{0:X8}' -f $cgRestored);CpaStageMask=('0x{0:X8}' -f $cpaStageMask)
  }|ConvertTo-Json -Depth 10|Set-Content (Join-Path $dir 'h15o_transaction.json') -Encoding UTF8
  $finalGctl=U32 $r 356;$finalEm2=U32 $r 368
  $writeRestoreComplete=(($fl -band [Convert]::ToUInt32('3FC00000',16)) -eq [Convert]::ToUInt32('3FC00000',16) -and $pgRestored -eq $ExpectedPg -and $cgRestored -eq $ExpectedCg -and $ppctlRestored -eq 0 -and $finalGctl -eq 0 -and $finalEm2 -eq 0x04007000)
  if($v -ne 1 -or $sz -ne 464 -or $nt -lt 0 -or ($fl -band $RequiredFlags) -ne $RequiredFlags -or $ven -ne 0x8086 -or $dev -ne 0x3198 -or
     $pg -ne $ExpectedPg -or $cg -ne $ExpectedCg -or $hda -ne $ExpectedHda -or $dsp -ne $ExpectedDsp -or $hdaLen -ne $ExpectedHdaLength -or $dspLen -ne $ExpectedDspLength -or
     $llch -ne 0x00000C00 -or $ppOffset -ne 0x00000800 -or $ppHeader -ne 0x00030500 -or $ppctlBefore -ne 0 -or ($ppctlEnabled -band 0x40000000) -eq 0 -or
     $pgAfterCg -ne 0x00000010 -or $cgAfterCg -ne 0x807B0DFD -or $pgAfterPg -ne 0x00000014 -or $cgAfterPg -ne 0x807B0DFD -or -not $writeRestoreComplete){
    throw 'H15O_LIVE_TRANSACTION_VALIDATION_FAILED'
  }
  $snapshotComplete=$true

  $x=PnP @('/delete-driver',$publishedInf,'/uninstall','/force')
  $x.Output|Set-Content (Join-Path $dir 'pnputil_remove_h15o.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15O_UNINSTALL_FAILED'}
  ...
  if(@(Published).Count -ne 0){throw 'H15O_PACKAGE_REMAINS'}
  ...
  if(-not $normal){
    ...
    if($published -and ((-not $writeAttempted) -or $writeRestoreComplete)){
      ...
      $log+='AUTOMATIC_UNINSTALL_BLOCKED_UNPROVEN_H15O_POLICY_RESTORE=TRUE'
      $log+='USE_H15O_WINRE_ROLLBACK_IF_WINDOWS_BECOMES_UNBOOTABLE=TRUE'
    }
...
$status=if($normal -and $baseline -and $handoffComplete -and $snapshotComplete -and -not $err){
  'H15O_STAGED_PRE_FW_POLICY_AND_ROLLBACK_COMPLETE'
}else{'H15O_STAGED_PRE_FW_POLICY_TRANSACTION_FAILED'}
...
[ordered]@{
  ...
  FunctionDriver='Phaser360H15o'
  Mapping='HDA_PAGE_READWRITE_DSP_PAGE_READONLY'
  MmioRead='HDA_DSP_STATUS_STAGED_GPROCEN_CGCTL_EM2_PGCTL_CPA0';MmioWrite='ONLY_HDA_GCTL_CRST_PPCTL_GPROCEN_EM2_L1SEN_WITH_EXACT_ROLLBACK'
  PpctlWrite='ONLY_GPROCEN_BIT30_SET_CLEAR';Em2Write='ONLY_L1SEN_BIT13_CLEAR_SET';DspMmioWrite='NO';PciConfigWrite='ONLY_CGCTL_0x48_BIT1_AND_PGCTL_0x44_BIT2_WITH_EXACT_ROLLBACK';Dma='NO'
...
Hashes $dir
$zip=Join-Path $OutputRoot ('RESULT_H15O_TRANSACTION_'+$stamp+'_'+$suffix+'.zip')
