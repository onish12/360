...
$Service='Phaser360H15m'
$CertSubject='CN=PHASER360 H15M Ephemeral Test Signing'
$InterfaceGuid=[Guid]'8c1b3150-6d12-4f88-9d36-15f700319803'
$Ioctl=[Convert]::ToUInt32('8340E480',16)
$RequiredFlags=[Convert]::ToUInt32('07FFFCFF',16)
$ResultBytes=328
...
function Published {
  ...
  foreach($f in @(Get-ChildItem (Join-Path $env:SystemRoot 'INF') -Filter 'oem*.inf' -File)){
    ...
    if($t.IndexOf('Phaser360H15m',[StringComparison]::OrdinalIgnoreCase) -ge 0 -and
...
function CodeIntegrity {
  if(-not ('Phaser360.H15mCi' -as [type])){
    Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;namespace Phaser360{public static class H15mCi{[StructLayout(LayoutKind.Sequential)]struct CI{public UInt32 Length;public UInt32 Options;}[DllImport("ntdll.dll")]static extern Int32 NtQuerySystemInformation(Int32 c,ref CI i,UInt32 l,IntPtr r);public static UInt32 Get(){CI i=new CI();i.Length=(UInt32)Marshal.SizeOf(typeof(CI));Int32 s=NtQuerySystemInformation(103,ref i,i.Length,IntPtr.Zero);if(s<0)throw new InvalidOperationException("CI=0x"+unchecked((UInt32)s).ToString("X8"));return i.Options;}}}' -Language CSharp
  }
  [uint32][Phaser360.H15mCi]::Get()
}
function Package([string]$root,[switch]$Trusted){
  $r=(Resolve-Path $root).Path
  $inf=Join-Path $r 'phaser360_h15m_hda_ppcap_probe.inf'
  $sys=Join-Path $r 'phaser360_h15m_hda_ppcap_probe.sys'
  $cat=Join-Path $r 'phaser360_h15m_hda_ppcap_probe.cat'
  $cer=Join-Path $r 'phaser360_h15m_hda_ppcap_probe.cer'
  ...
  if([string]$m.Purpose -cne 'H15M_PPCAP_DISCOVERY_TRANSACTION_PACKAGE' -or
    ...
    throw 'MANIFEST_H15M_CONTRACT_MISMATCH'
  }
  foreach($x in @(
...
function Native {
  if('Phaser360.H15mNative' -as [type]){return}
  $src=@'
using System;
...
namespace Phaser360 {
  public static class H15mNative {
    ...
    static string InterfacePath(Guid g) {
      ...
      try {
        ...
        if(second||Marshal.GetLastWin32Error()!=ERROR_NO_MORE_ITEMS)
          throw new InvalidOperationException("EXPECTED_EXACTLY_ONE_H15M_INTERFACE");
        uint n=0;
    ...
    public static byte[] Transaction(Guid g,uint ioctl,byte[] input,int bytes) {
      ...
      using(var h=CreateFile(path,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ|FILE_SHARE_WRITE,IntPtr.Zero,OPEN_EXISTING,0,IntPtr.Zero)) {
        if(h.IsInvalid) throw new Win32Exception(Marshal.GetLastWin32Error(),"CreateFile H15M");
        var o=new byte[bytes]; uint got;
        Q(DeviceIoControl(h,ioctl,input,(uint)input.Length,o,(uint)o.Length,out got,IntPtr.Zero),"DeviceIoControl H15M");
        if(got!=bytes) throw new InvalidOperationException("H15M_RESULT_SIZE="+got);
        return o;
      }
    }
  }
...
if(@(Published).Count -ne 0){throw 'H15M_PACKAGE_ALREADY_PRESENT'}
...
$tb=Trust $pkg.Thumb
if($tb.Root -or $tb.TrustedPublisher){throw 'H15M_CERT_ALREADY_TRUSTED'}

if([string]::IsNullOrWhiteSpace($OutputRoot)){$OutputRoot=$PSScriptRoot}
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$suffix=[Guid]::NewGuid().ToString('N').Substring(0,8)
$dir=Join-Path $OutputRoot ('H15M_TRANSACTION_'+$stamp+'_'+$suffix)
...
Copy-Item (Join-Path $PSScriptRoot 'H15M_WINRE_ROLLBACK.txt') $dir
...
try {
  ...
  $x.Output|Set-Content (Join-Path $dir 'pnputil_publish_h15m.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15M_DRIVERSTORE_PUBLISH_FAILED'}
  $p=@(Published)
  if($p.Count -ne 1){throw "EXPECTED_ONE_H15M_PUBLISHED_INF: count=$($p.Count)"}
  $publishedInf=$p[0]
  $published=$true
  $publishedInf|Set-Content (Join-Path $dir 'H15mPublishedInf.txt') -Encoding ASCII

  Native
  $handoffAttempted=$true
  $reboot=[Phaser360.H15mNative]::ForceUpdate($ExactHwid,$pkg.Inf)
  if($reboot){throw 'H15M_HANDOFF_REQUIRES_SYSTEM_REBOOT'}

  $with=WaitService $before.InstanceId $Service 20
  $with|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'target_with_h15m.json') -Encoding UTF8
  if($with.DriverInfPath -cne $publishedInf){
    throw "H15M_PUBLISHED_INF_NOT_ACTIVE: active=$($with.DriverInfPath) published=$publishedInf"
  }
  ...
  $writeAttempted=$true
  $r=[Phaser360.H15mNative]::Transaction($InterfaceGuid,$Ioctl,$req,$ResultBytes)
  ...
  function Obs([byte[]]$b,[int]$o){
    [ordered]@{
      HdaGcap=('0x{0:X4}' -f (U16 $b $o))
      HdaVmin=('0x{0:X2}' -f [uint32]$b[$o+2]);HdaVmaj=('0x{0:X2}' -f [uint32]$b[$o+3])
      HdaGctl=('0x{0:X8}' -f (U32 $b ($o+4)))
      HdaCorbctl=('0x{0:X2}' -f [uint32]$b[$o+8]);HdaRirbctl=('0x{0:X2}' -f [uint32]$b[$o+9])
      TotalStreams=[uint32]$b[$o+10];StreamRunMask=('0x{0:X8}' -f (U32 $b ($o+12)))
      HdaIntelEm2=('0x{0:X8}' -f (U32 $b ($o+16)))
      DspAdspcs=('0x{0:X8}' -f (U32 $b ($o+20)));DspAdspic=('0x{0:X8}' -f (U32 $b ($o+24)))
      DspAdspis=('0x{0:X8}' -f (U32 $b ($o+28)));DspHipci=('0x{0:X8}' -f (U32 $b ($o+32)))
      DspHipcie=('0x{0:X8}' -f (U32 $b ($o+36)));DspHipcctl=('0x{0:X8}' -f (U32 $b ($o+40)))
      DspRomStatus=('0x{0:X8}' -f (U32 $b ($o+44)))
    }
  }
  $beforeRegs=Obs $r 64;$readyRegs=Obs $r 112;$restoredRegs=Obs $r 160
  $llch=U32 $r 208;$ppOffset=U32 $r 212;$ppHeader=U32 $r 216;$ppctl=U32 $r 220;$ppsts=U32 $r 224;$capCount=U32 $r 228
  $caps=@()
  $capTake=[Math]::Min([int]$capCount,12)
  for($i=0;$i -lt $capTake;$i++){
    $o=232+($i*8)
    $hcap=U32 $r ($o+4)
    $caps+=[ordered]@{Index=$i;Offset=('0x{0:X4}' -f (U32 $r $o));Header=('0x{0:X8}' -f $hcap);Id=(($hcap-shr 16)-band 0xFFF);Next=($hcap-band 0xFFFF)}
  }
  $ppFound=(($fl -band 0x00000100) -ne 0)
  $gprocen=(($ppctl -band 0x40000000) -ne 0)
  $pie=(($ppctl -band 0x80000000) -ne 0)
  ...
  [ordered]@{
    ...
    Before=$beforeRegs;Ready=$readyRegs;Restored=$restoredRegs
    Llch=('0x{0:X8}' -f $llch);CapabilityCount=$capCount;Capabilities=$caps
    PpFound=$ppFound;PpOffset=('0x{0:X4}' -f $ppOffset);PpHeader=('0x{0:X8}' -f $ppHeader)
    Ppctl=('0x{0:X8}' -f $ppctl);Ppsts=('0x{0:X8}' -f $ppsts);Gprocen=$gprocen;Pie=$pie
  }|ConvertTo-Json -Depth 10|Set-Content (Join-Path $dir 'h15m_transaction.json') -Encoding UTF8

  $beforeGctl=U32 $r 68;$readyGctl=U32 $r 116;$restoredGctl=U32 $r 164
  $writeRestoreComplete=(($fl -band [Convert]::ToUInt32('00003C00',16)) -eq [Convert]::ToUInt32('00003C00',16) -and $restoredGctl -eq $beforeGctl)
  if($v -ne 1 -or $sz -ne 328 -or $nt -lt 0 -or
     ...
     $beforeGctl -ne 0 -or ($readyGctl -band 1) -ne 1 -or -not $writeRestoreComplete -or $capCount -lt 1 -or $capCount -gt 12){
    throw 'H15M_LIVE_TRANSACTION_VALIDATION_FAILED'
  }
  $snapshotComplete=$true

  $x=PnP @('/delete-driver',$publishedInf,'/uninstall','/force')
  $x.Output|Set-Content (Join-Path $dir 'pnputil_remove_h15m.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15M_UNINSTALL_FAILED'}
  ...
  if(@(Published).Count -ne 0){throw 'H15M_PACKAGE_REMAINS'}
  ...
  if(-not $normal){
    ...
    if($published -and ((-not $writeAttempted) -or $writeRestoreComplete)){
      ...
      $log+='USE_H15M_WINRE_ROLLBACK_IF_WINDOWS_BECOMES_UNBOOTABLE=TRUE'
    }
...
$status=if($normal -and $baseline -and $handoffComplete -and $snapshotComplete -and -not $err){
  'H15M_PPCAP_DISCOVERY_AND_GCTL_ROLLBACK_COMPLETE'
}else{'H15M_PPCAP_DISCOVERY_FAILED'}
...
[ordered]@{
  ...
  FunctionDriver='Phaser360H15m'
  Mapping='HDA_PAGE_READWRITE_DSP_PAGE_READONLY'
  MmioRead='HDA_DSP_STATUS_AND_CAPABILITY_CHAIN_PPCTL_PPSTS';MmioWrite='ONLY_HDA_GCTL_CRST_BIT0_0_TO_1_TO_0'
  PpctlWrite='NO';Em2Write='NO';DspMmioWrite='NO';PciConfigWrite='NO';Dma='NO'
...
Hashes $dir
$zip=Join-Path $OutputRoot ('RESULT_H15M_TRANSACTION_'+$stamp+'_'+$suffix+'.zip')
