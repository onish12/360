...
$ProbeService='Phaser360H15or';$H15oService='Phaser360H15o'
$ProbeSubject='CN=PHASER360 H15OR Readonly Recovery Signing'
...
function Package([string]$root,[switch]$Trusted){
 $r=(Resolve-Path $root).Path;$sys=Join-Path $r 'phaser360_h15or_readonly_recovery_probe.sys';$cer=Join-Path $r 'phaser360_h15or_readonly_recovery_probe.cer';$man=Join-Path $r 'package_manifest.json'
 ...
 if([string]$m.Purpose -cne 'H15OR_READONLY_RECOVERY_PACKAGE' -or [string]$m.SysSha256 -cne (Get-FileHash $sys -Algorithm SHA256).Hash.ToLowerInvariant()){throw 'RECOVERY_MANIFEST_INVALID'}
...
if(IsIntel $before){Write-Host 'STATUS=H15OR_ALREADY_INTEL_BASELINE';exit 0}
...
try{
 $x=CertUtil @('-f','-addstore','Root',$pkg.Cer);if($x.ExitCode-ne0){throw 'H15OR_CERT_ROOT_ADD_FAILED'};$probeRoot=$true
