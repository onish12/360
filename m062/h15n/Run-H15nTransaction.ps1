...
$Service='Phaser360H15n'
$CertSubject='CN=PHASER360 H15N Ephemeral Test Signing'
$InterfaceGuid=[Guid]'8c1b3150-6d12-4f88-9d36-15f800319804'
$Ioctl=[Convert]::ToUInt32('8341E484',16)
$RequiredFlags=[Convert]::ToUInt32('FFFFEFFF',16)
$ResultBytes=344
...
function Published {
  ...
  foreach($f in @(Get-ChildItem (Join-Path $env:SystemRoot 'INF') -Filter 'oem*.inf' -File)){
    ...
    if($t.IndexOf('Phaser360H15n',[StringComparison]::OrdinalIgnoreCase) -ge 0 -and
...
function CodeIntegrity {
  if(-not ('Phaser360.H15nCi' -as [type])){
    Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;namespace Phaser360{public static class H15nCi{[StructLayout(LayoutKind.Sequential)]struct CI{public UInt32 Length;public UInt32 Options;}[DllImport("ntdll.dll")]static extern Int32 NtQuerySystemInformation(Int32 c,ref CI i,UInt32 l,IntPtr r);public static UInt32 Get(){CI i=new CI();i.Length=(UInt32)Marshal.SizeOf(typeof(CI));Int32 s=NtQuerySystemInformation(103,ref i,i.Length,IntPtr.Zero);if(s<0)throw new InvalidOperationException("CI=0x"+unchecked((UInt32)s).ToString("X8"));return i.Options;}}}' -Language CSharp
  }
  [uint32][Phaser360.H15nCi]::Get()
}
function Package([string]$root,[switch]$Trusted){
  $r=(Resolve-Path $root).Path
  $inf=Join-Path $r 'phaser360_h15n_ppctl_gprocen_gate.inf'
  $sys=Join-Path $r 'phaser360_h15n_ppctl_gprocen_gate.sys'
  $cat=Join-Path $r 'phaser360_h15n_ppctl_gprocen_gate.cat'
  $cer=Join-Path $r 'phaser360_h15n_ppctl_gprocen_gate.cer'
  ...
  if([string]$m.Purpose -cne 'H15N_PPCTL_GPROCEN_TRANSACTION_PACKAGE' -or
     ...
     [string]$m.MmioWrite -cne 'ONLY_HDA_GCTL_CRST_AND_PPCTL_GPROCEN_BIT30_WITH_EXACT_ROLLBACK' -or
     [string]$m.PpctlWrite -cne 'ONLY_GPROCEN_BIT30_SET_CLEAR' -or
     [string]$m.Em2Write -cne 'NO' -or
    ...
    throw 'MANIFEST_H15N_CONTRACT_MISMATCH'
  }
  foreach($x in @(
...
function Native {
  if('Phaser360.H15nNative' -as [type]){return}
  $src=@'
using System;
...
namespace Phaser360 {
  public static class H15nNative {
    ...
    static string InterfacePath(Guid g) {
      ...
      try {
        ...
        if(second||Marshal.GetLastWin32Error()!=ERROR_NO_MORE_ITEMS)
          throw new InvalidOperationException("EXPECTED_EXACTLY_ONE_H15N_INTERFACE");
        uint n=0;
    ...
    public static byte[] Transaction(Guid g,uint ioctl,byte[] input,int bytes) {
      ...
      using(var h=CreateFile(path,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ|FILE_SHARE_WRITE,IntPtr.Zero,OPEN_EXISTING,0,IntPtr.Zero)) {
        if(h.IsInvalid) throw new Win32Exception(Marshal.GetLastWin32Error(),"CreateFile H15N");
        var o=new byte[bytes]; uint got;
        Q(DeviceIoControl(h,ioctl,input,(uint)input.Length,o,(uint)o.Length,out got,IntPtr.Zero),"DeviceIoControl H15N");
        if(got!=bytes) throw new InvalidOperationException("H15N_RESULT_SIZE="+got);
        return o;
      }
    }
  }
...
if(@(Published).Count -ne 0){throw 'H15N_PACKAGE_ALREADY_PRESENT'}
...
$tb=Trust $pkg.Thumb
if($tb.Root -or $tb.TrustedPublisher){throw 'H15N_CERT_ALREADY_TRUSTED'}

if([string]::IsNullOrWhiteSpace($OutputRoot)){$OutputRoot=$PSScriptRoot}
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'
$suffix=[Guid]::NewGuid().ToString('N').Substring(0,8)
$dir=Join-Path $OutputRoot ('H15N_TRANSACTION_'+$stamp+'_'+$suffix)
...
Copy-Item (Join-Path $PSScriptRoot 'H15N_WINRE_ROLLBACK.txt') $dir
...
try {
  ...
  $x.Output|Set-Content (Join-Path $dir 'pnputil_publish_h15n.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15N_DRIVERSTORE_PUBLISH_FAILED'}
  $p=@(Published)
  if($p.Count -ne 1){throw "EXPECTED_ONE_H15N_PUBLISHED_INF: count=$($p.Count)"}
  $publishedInf=$p[0]
  $published=$true
  $publishedInf|Set-Content (Join-Path $dir 'H15nPublishedInf.txt') -Encoding ASCII

  Native
  $handoffAttempted=$true
  $reboot=[Phaser360.H15nNative]::ForceUpdate($ExactHwid,$pkg.Inf)
  if($reboot){throw 'H15N_HANDOFF_REQUIRES_SYSTEM_REBOOT'}

  $with=WaitService $before.InstanceId $Service 20
  $with|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dir 'target_with_h15n.json') -Encoding UTF8
  if($with.DriverInfPath -cne $publishedInf){
    throw "H15N_PUBLISHED_INF_NOT_ACTIVE: active=$($with.DriverInfPath) published=$publishedInf"
  }
  ...
  $writeAttempted=$true
  $r=[Phaser360.H15nNative]::Transaction($InterfaceGuid,$Ioctl,$req,$ResultBytes)

  $v=U32 $r 0;$sz=U32 $r 4;$nt=I32 $r 8;$fl=U32 $r 12;$gen=U32 $r 16
  $ven=U16 $r 20;$dev=U16 $r 22;$pg=U32 $r 28;$cg=U32 $r 32
  $hda=U64 $r 40;$dsp=U64 $r 48;$hdaLen=U32 $r 56;$dspLen=U32 $r 60
  ...
  $beforeRegs=Obs $r 64;$readyRegs=Obs $r 112;$enabledRegs=Obs $r 160;$disabledRegs=Obs $r 208;$finalRegs=Obs $r 256
  $llch=U32 $r 304;$ppOffset=U32 $r 308;$ppHeader=U32 $r 312
  $ppctlBefore=U32 $r 316;$ppstsBefore=U32 $r 320;$ppctlEnabled=U32 $r 324;$ppstsEnabled=U32 $r 328;$ppctlDisabled=U32 $r 332;$ppstsDisabled=U32 $r 336
  $cpa0Observed=(($fl -band 0x00001000) -ne 0)
  [ordered]@{
    Version=$v;Size=$sz;NtStatus=('0x{0:X8}' -f ([uint32]$nt));Flags=('0x{0:X8}' -f $fl);Generation=$gen
    Vendor=('0x{0:X4}' -f $ven);Device=('0x{0:X4}' -f $dev);Pgctl=('0x{0:X8}' -f $pg);Cgctl=('0x{0:X8}' -f $cg)
    HdaPhysical=('0x{0:X16}' -f $hda);DspPhysical=('0x{0:X16}' -f $dsp);HdaLength=('0x{0:X8}' -f $hdaLen);DspLength=('0x{0:X8}' -f $dspLen)
    Before=$beforeRegs;Ready=$readyRegs;Enabled=$enabledRegs;Disabled=$disabledRegs;Final=$finalRegs
    Llch=('0x{0:X8}' -f $llch);PpOffset=('0x{0:X4}' -f $ppOffset);PpHeader=('0x{0:X8}' -f $ppHeader)
    PpctlBefore=('0x{0:X8}' -f $ppctlBefore);PpstsBefore=('0x{0:X8}' -f $ppstsBefore)
    PpctlEnabled=('0x{0:X8}' -f $ppctlEnabled);PpstsEnabled=('0x{0:X8}' -f $ppstsEnabled)
    PpctlDisabled=('0x{0:X8}' -f $ppctlDisabled);PpstsDisabled=('0x{0:X8}' -f $ppstsDisabled);Cpa0Observed=$cpa0Observed
  }|ConvertTo-Json -Depth 10|Set-Content (Join-Path $dir 'h15n_transaction.json') -Encoding UTF8
  $beforeGctl=U32 $r 68;$finalGctl=U32 $r 260
  $writeRestoreComplete=(($fl -band [Convert]::ToUInt32('001EE000',16)) -eq [Convert]::ToUInt32('001EE000',16) -and $ppctlDisabled -eq $ppctlBefore -and $finalGctl -eq $beforeGctl)
  if($v -ne 1 -or $sz -ne 344 -or $nt -lt 0 -or ($fl -band $RequiredFlags) -ne $RequiredFlags -or
     $ven -ne 0x8086 -or $dev -ne 0x3198 -or $pg -ne $ExpectedPg -or $cg -ne $ExpectedCg -or
     $hda -ne $ExpectedHda -or $dsp -ne $ExpectedDsp -or $hdaLen -ne $ExpectedHdaLength -or $dspLen -ne $ExpectedDspLength -or
     $llch -ne 0x00000C00 -or $ppOffset -ne 0x00000800 -or $ppHeader -ne 0x00030500 -or
     $ppctlBefore -ne 0 -or ($ppctlEnabled -band 0x40000000) -eq 0 -or $ppctlDisabled -ne 0 -or -not $writeRestoreComplete){
    throw 'H15N_LIVE_TRANSACTION_VALIDATION_FAILED'
  }
  $snapshotComplete=$true

  $x=PnP @('/delete-driver',$publishedInf,'/uninstall','/force')
  $x.Output|Set-Content (Join-Path $dir 'pnputil_remove_h15n.txt') -Encoding UTF8
  if($x.ExitCode -ne 0){throw 'H15N_UNINSTALL_FAILED'}
  ...
  if(@(Published).Count -ne 0){throw 'H15N_PACKAGE_REMAINS'}
  ...
  if(-not $normal){
    ...
    if($published -and ((-not $writeAttempted) -or $writeRestoreComplete)){
      ...
      $log+='AUTOMATIC_UNINSTALL_BLOCKED_UNPROVEN_PPCTL_OR_GCTL_RESTORE=TRUE'
      $log+='USE_H15N_WINRE_ROLLBACK_IF_WINDOWS_BECOMES_UNBOOTABLE=TRUE'
    }
...
$status=if($normal -and $baseline -and $handoffComplete -and $snapshotComplete -and -not $err){
  'H15N_PPCTL_GPROCEN_AND_GCTL_ROLLBACK_COMPLETE'
}else{'H15N_PPCTL_GPROCEN_TRANSACTION_FAILED'}
...
[ordered]@{
  ...
  FunctionDriver='Phaser360H15n'
  Mapping='HDA_PAGE_READWRITE_DSP_PAGE_READONLY'
  MmioRead='HDA_DSP_STATUS_PPCTL_PPSTS_CPA0';MmioWrite='ONLY_HDA_GCTL_CRST_AND_PPCTL_GPROCEN_BIT30_WITH_EXACT_ROLLBACK'
  PpctlWrite='ONLY_GPROCEN_BIT30_SET_CLEAR';Em2Write='NO';DspMmioWrite='NO';PciConfigWrite='NO';Dma='NO'
...
Hashes $dir
$zip=Join-Path $OutputRoot ('RESULT_H15N_TRANSACTION_'+$stamp+'_'+$suffix+'.zip')
