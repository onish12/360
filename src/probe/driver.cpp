...
WDF_DECLARE_CONTEXT_TYPE_WITH_NAME(PHASER_DEVICE_CONTEXT, PhaserGetDeviceContext)
...
static void PhaserLog(_In_z_ PCSTR Message)
{
...
)
{
    ...
    pnp.EvtDevicePrepareHardware = PhaserEvtDevicePrepareHardware;
    pnp.EvtDeviceReleaseHardware = PhaserEvtDeviceReleaseHardware;
    WdfDeviceInitSetPnpPowerEventCallbacks(DeviceInit, &pnp);

    WDF_OBJECT_ATTRIBUTES attributes;
    WDF_OBJECT_ATTRIBUTES_INIT_CONTEXT_TYPE(&attributes, PHASER_DEVICE_CONTEXT);
    ...
    PhaserLog("device created; no hardware access performed");
...
)
{
    ...
    auto ctx = PhaserGetDeviceContext(Device);
    RtlZeroMemory(&ctx->Adsp, sizeof(ctx->Adsp));
    ...
    if (ctx->Adsp.Version != PHASER_ADSP_INTERFACE_VERSION ||
    ...
    if (ctx->Adsp.CtlrDevId != PHASER_GEMINI_LAKE_AUDIO_DEVICE_ID) {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "PHASER360_PROBE: unexpected controller 0x%04hX (expected 0x3198)\n",
            ctx->Adsp.CtlrDevId));
        return STATUS_DEVICE_HARDWARE_ERROR;
    }

    // M0.1 stops here. None of the returned hardware callback pointers are called.
    PhaserLog("PASS: Gemini Lake ADSP interface acquired; zero hardware callbacks invoked");
    return STATUS_SUCCESS;
}
...
)
{
    ...
    auto ctx = PhaserGetDeviceContext(Device);
    if (ctx->InterfaceAcquired && ctx->Adsp.InterfaceDereference != nullptr) {
        ctx->Adsp.InterfaceDereference(ctx->Adsp.Context);
    }

    RtlZeroMemory(&ctx->Adsp, sizeof(ctx->Adsp));
    ctx->InterfaceAcquired = FALSE;
    PhaserLog("interface released");
    return STATUS_SUCCESS;
}
